...
private:
    ...
    std::pair<float, float> read_battery(int repetitions = 1) {
        float rd_factor = 2.0;
        float max_vbat = 4.0;
        float min_vbat = 3.0;
        float bl = 0.0;
        ...
        return std::make_pair(average_battery_level, average_battery_percentage);
    }

    // Check battery status and emit updates
    void check_battery_status() {
        try {
            auto [battery_level, battery_percentage] = read_battery();

            BatteryStatus new_status;
            if (battery_level > config.voltage_hi) {
                new_status = BatteryStatus::GREEN;
            } else if (battery_level > config.low_threshold) {
                new_status = BatteryStatus::YELLOW;
            } else if (battery_level > config.critical_threshold) {
            ...
            if (now - last_log_time >= std::chrono::minutes(1)) {
                ...
                line_stream << current_counter << ", "
                            << dts << ", "
                            << std::fixed << std::setprecision(2) << battery_level << ", "
    ...
    void shutdown_board() {
        ...
        auto [battery_level, battery_percentage] = read_battery(REPEAT_INIT);
        if (battery_level < config.voltage_hi) {
